...
// add the sub resource request to the list so they will be inluded in the linked resources
...
// add products to subResourceRequests
utilities._addProducts = function _addProducts(stateObject, productResourceName, productIdsToAdd, existingProductResourceName) {
  if (!_.isArray(productIdsToAdd) || _.isEmpty(productIdsToAdd) || _.isUndefined(productResourceName)) {
    return false;
  }

  productIdsToAdd = utilities._removeExistingProductsFromGhostproducts(productIdsToAdd, _.get(stateObject, ['subResourceRequests', existingProductResourceName,'ids']));

  // just in case ghostHotelProducts does not exists
  if (!_.get(stateObject, ['subResourceRequests', productResourceName, 'ids'])) {
    _.set(stateObject, ['subResourceRequests', productResourceName, 'ids'], []);
  }

  stateObject.subResourceRequests[productResourceName].ids = _.union(stateObject.subResourceRequests[productResourceName].ids, productIdsToAdd);

  return false;
};

// remove existing products from the ghost products
utilities._removeExistingProductsFromGhostproducts = function _removeExistingProductsFromGhostproducts(ghostProducts, existingProducts) {
  if (_.isEmpty(existingProducts) || _.isEmpty(ghostProducts)) {
    return ghostProducts;
  }

  return _.reduce(ghostProducts, function(result, ghostProduct) {
    if (!_.includes(existingProducts, ghostProduct)) {
      // the product does not come back, so we can ghost it
      result.push(ghostProduct);
    }
...
// add ghost products to subResourceRequests
utilities.addGhostProducts = function addGhostProducts(stateObject) {
  // hardcode the chessington hotels/rooms for the ghost hotels
  if (_.get(stateObject.request, 'data.referrer') === 'chessington' && _.get(stateObject.request, 'path') === '/packageRates/') {
    if (_.includes(stateObject.includes, 'ghostHotelProducts')) {
      // add the ghost hotels
      var chessingtonGhostHotels = [
        'RESSAF',
        'RESAZT',
        'GLACHE'
      ];
      utilities._addProducts(stateObject, 'ghostHotelProducts', chessingtonGhostHotels, 'hotelProducts');
    }

    if (_.includes(stateObject.includes, 'ghostRoomProducts')) {
      // add the ghost rooms
      var chessingtonGhostRooms = ['CHEGRU'];
      utilities._addProducts(stateObject, 'ghostRoomProducts', chessingtonGhostRooms, 'roomProducts');
    }
  }
