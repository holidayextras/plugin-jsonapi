...
console.log( ' -- plugin - resolveLinkedData - stateObject.request.data.referrer -- ', stateObject.request.data.referrer );

console.log( ' -- plugin - resolveLinkedData - stateObject.subResourceRequests -- ', stateObject.subResourceRequests );
// stateObject.subResourceRequests.hotelProducts.ids
// stateObject.subResourceRequests.roomProducts.ids


// TODO need to check that hotel products are in the includes list

// hardcode the chessington hotels for the ghost hotels
if (_.get(stateObject.request, 'data.referrer') === 'chessington') {
  // just in case hotelProducts does not exists
  if (_.isUndefined(stateObject.subResourceRequests.hotelProducts)) {
    stateObject.subResourceRequests.hotelProducts = {
      ids: [];
    }
  }

  stateObject.subResourceRequests.hotelProducts.ids.push('RESSAF');
}
