...
utilities.getResourcesLinks = function getResourcesLinks(links, includes) {
...
utilities.getResources = function getResources( stateObject ) {
  ...
  _.each(resourceArray, function( resource ) {
    ...
    if (resource.links) {
      utilities.getResourcesLinks(resource.links, stateObject.includes);
    }
  });
...
// -------------------
//  // Loop the resources and pad out any required hrefs
//  stateObject.subResourceRequests = _.reduce(resourceArray, function(result, resource) {
//    // If there's no links then there's no actions to take
//    if (resource.links) {
//      // Loop all the includes and pad out our subResourceRequest array
//      _.each(stateObject.includes, function( include ) {
//        // If we have links to the requested include, chalk it up
//        if (resource.links && resource.links[include]) {
//          utilities._addSubResourceRequest(result, resource.links[include]);
//        }
//      });
//    }
//  }, {});
// -------------------
  ...
  console.log( ' -- stateObject.subResourceRequests -- ', stateObject.subResourceRequests );

  return Q.resolve(stateObject);
};


utilities._resolveLinkedData = function _resolveLinkedData( stateObject ) {
  ...
  _.each(stateObject.subResourceRequests, function( info, type ) {
    ...
    var subResourceUrl = utilities._buildHref(hrefParameters, stateObject.request.url.query.context);


    var headerHost = _.get(stateObject.request, 'auth.artifacts.host');
    if (_.get(stateObject.request, 'auth.artifacts.port')) {
      headerHost += ':' + stateObject.request.auth.artifacts.port;
    }

    stateObject.request.server.inject({
      url: subResourceUrl,
      credentials: stateObject.request.auth.credentials,
      artifacts: stateObject.request.auth.artifacts,
      headers: {
        host: headerHost
      }
    }, function( response ) {
...
utilities.cleanUpResources = function cleanUpResources(stateObject) {
