...
function pluginJsonapi( server, options, next ) {
  ...
  */
  function _buildHref( requestData, context ) {
    var ids = _.isArray( requestData.ids ) ? requestData.ids.join( ',' ) : requestData.ids;

    var queryString = '';
    var query = {};

    if ( !_.isUndefined( context ) && !_.isUndefined( context[requestData.type] ) ) {
      // build the url parts of context
      query.context = {};
      query.context[requestData.type] = context[requestData.type];
    }

    if ( !_.isUndefined( requestData.filter ) ) {
      // add the filters to the query
      query.filter = requestData.filter;
    }
    ...
    return url.format( {
      pathname: '/' + path.join( requestData.type, ids ),
      search: queryString
    } );
  }
  ...
  */
  function _resolveLinkedData( request, subResourceRequests ) {
    ...
    _.each( subResourceRequests, function( info, type ) {
      ...
      if ( info.filter ) {
        // add the filters
        requestData.filter = info.filter;
      }

      // Where will we find this resource?
      // if a `context` is available, proxy it to sub resources
      var subResourceUrl = _buildHref( requestData, request.url.query.context );
      ...
      if ( !request.response.isBoom && ( !_.contains( ignorePaths, request.route.path ) ) && ( !_.contains( ignoreStatusCodes, request.response.statusCode ) ) ) {
        ...
        if ( resources ) {
          ...
          _.each( resourceArray, function( resource ) {
            ...
            if ( resource.id ) {
              resource.href = _buildHref( { type: request.route.settings.bind.resourceName, ids: resource.id } );
            }
            ...
            if ( resource.links ) {
              ...
              _.each( resource.links, function( value ) {
                ...
                if ( value.type && ( _.size( value.ids ) || _.size( value.filter ) ) ) {
                  ...
                  if ( _.size( value.ids ) ) {
                    requestData.ids = value.ids;
                  }

                  if ( _.size( value.filter ) ) {
                    requestData.filter = _.cloneDeep( value.filter );
                  ...
                  value.href = _buildHref( requestData );
                }
              } );
