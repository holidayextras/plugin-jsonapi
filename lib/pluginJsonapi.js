...
//      }
      if (!request.response.isBoom && !_.includes(ignorePaths, request.route.path) && !_.includes(ignoreStatusCodes, request.response.statusCode)) {
        ...
        /*
         * Flow
         * TODO write flow description
         */
        utilities.collectIncludes(stateObject)
        .then(utilities.getSubResources) // might do the subResources first so i can remove the filters from links
        .then(utilities.getResources)
        .then(utilities._resolveLinkedData)
        .then(utilities.cleanUpResources)
        .then(function(stateObject) {
...
//          console.log(' -- stateObject -- ', stateObject);
      ...
      console.log( ' -- plugin-jsonapi - error -- ', error );
